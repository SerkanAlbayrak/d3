...
Katsayılar hesaplanmadan önce verideki yerleştirme puanlarından OBP etkisinin ayıklanması gerekmekte. OBP etkisi öğrencinin mezun olduğu lise ile yerleştiği program arasında bir bağlantı olmasına göre 0.18 ile 0.12 değerlerinden birini alıyor. Ayrıca katsayılardan bağımsız OSYM tarafından eklenen 100 puan da çıkarılıyor.

$$
TYT_{Sınav} = TYT_{Yerleştirme}-(0.12\: veya\: 0.18)\cdot OBP-100
$$

```{r}
all$PuanS <- all$puan-all$Katsayı*all$obp-100
```

Verinin TYT ile yerleştirme yapılan kısmı filtrelenerek katsayılar aşağıda kurulan model ile tahminleniyor. Model veriyi %97.8 oranda başarıyla açıklıyor ancak geri kalan %2.2 fark, yuvarlama hatasından kaynaklanmak için fazla büyük. Daha sonra geri dönmek üzere şimdilik bu farkın yuvarlama hatasından kaynaklandığı varsayılmıştır. Katsayılar incelendiğinde soru bazında puana en fazla etkili soruların Temel Matematik Soruları olduğu görülüyor. Bir temel matematik sorusunun TYT puanına etkisi 4.78 puan olarak tahmin edilmektedir[^5].
...
library(jtools)
TYTdata <- all[all$`Puan Türü`=="TYT",] # TYT ile yerleşilen programlar
...
[^5]:Modelin hata terimleri incelendiğinde açıklanmaya muhtaç durumların olduğu fark ediliyor. Bu durum başka bir yazının konusu.
