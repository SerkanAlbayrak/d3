...
SAYısal puanlarının da hesaplanmasında kullanılan katsayılar benzer şekilde bulunabilir. SAY puanın TYT puanı ve AYT'den Matematik, Fizik, Kimya ve Biyoloji sorularından oluştuğu biliniyor. Kurulan modelde herhangi bir belirsizlik bulunmuyor. Veri %100 başarıyla açıklanabiliyor.  
...
ggplot(results, aes(x=real, y=fark, colour=absfark))+geom_point() + 
  ...
  geom_abline(slope = 0, intercept = 0, colour="orange", size=1)
```
Dil puanını için yapılan modelde yapılan hatanın 5'ten büyük olduğu çok sayıda durum olduğu görülüyor. 

```{r}
h <- which(results$absfark>5)
t(modeldataDIL[h,-c(13:24,28,30)]) |> kbl()
```
Dil modeliyle ilgili gözden kaçan bir durum olduğu açık. Henüz problemin kaynağı bulunabilmiş değil. Yukarıda sunulan model eldekiler arasında mevcut veriyi en başarılı şekilde açıklayabilen modeldir.

# Sonuç ve Tartışma

Bu yazıda hesaplanan model parametreleri ilgili puan türünde her soru grubunda yapılan bir netin kaç puana denk geldiğini göstermektedir. DİL puanının hesaplanmasına yönelik bir problemla karşılaşıldı. Şu an için bir çözüm getirilemedi. Genel olarak puan hesaplanma yöntemine yönelik tartışma yazının ikinci bölümüne ertelendi. 
